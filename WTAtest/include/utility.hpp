...
template <typename T>
__device__ inline uint64_t pack_uint16x4(T x, T y, T z, T w) {
    ushort4 uint16x4;
    uint16x4.x = static_cast<uint16_t>(x);
    uint16x4.y = static_cast<uint16_t>(y);
    uint16x4.z = static_cast<uint16_t>(z);
    uint16x4.w = static_cast<uint16_t>(w);
    return load_as<uint64_t>(&uint16x4);
}
...
template <unsigned int N>
__device__ inline void store_uint16_vector1(uint16_t *dest, const uint32_t *ptr);
template <>
__device__ inline void store_uint16_vector1<16u>(uint16_t *dest, const uint32_t *ptr) {
  
    ulonglong4 uint64x4;
    uint64x4.x = pack_uint16x4(ptr[0], ptr[1], ptr[2], ptr[3]);
    uint64x4.y = pack_uint16x4(ptr[4], ptr[5], ptr[6], ptr[7]);
    uint64x4.z = pack_uint16x4(ptr[8], ptr[9], ptr[10], ptr[11]);
    uint64x4.w = pack_uint16x4(ptr[12], ptr[13], ptr[14], ptr[15]);

    store_as<ulonglong4>(dest, uint64x4);
}
...
__device__ inline void copy_data(uint8_t *dest, const uint8_t *ptr);

template <>
__device__ inline void copy_data<1u>(uint8_t *dest, const uint8_t *ptr) {
    dest[0] = static_cast<uint8_t>(ptr[0]);
}

template <>
__device__ inline void copy_data<2u>(uint8_t *dest, const uint8_t *ptr) {
    uchar2 uchar8x2;
    uchar8x2.x = static_cast<uint8_t>(ptr[0]);
    uchar8x2.y = static_cast<uint8_t>(ptr[1]);
    store_as<uchar2>(dest, uchar8x2);
}

template <>
__device__ inline void copy_data<4u>(uint8_t *dest, const uint8_t *ptr) {
    uchar4 uchar8x4;
    uchar8x4.x = static_cast<uint8_t>(ptr[0]);
    uchar8x4.y = static_cast<uint8_t>(ptr[1]);
    uchar8x4.z = static_cast<uint8_t>(ptr[2]);
    uchar8x4.w = static_cast<uint8_t>(ptr[3]);
    store_as<uchar4>(dest, uchar8x4);
}

template <>
__device__ inline void copy_data<8u>(uint8_t *dest, const uint8_t *ptr) {
    ushort4 ushot16x4;
    copy_data<2u>(reinterpret_cast<uint8_t *>(&ushot16x4.x), &ptr[0]);
    copy_data<2u>(reinterpret_cast<uint8_t *>(&ushot16x4.y), &ptr[2]);
    copy_data<2u>(reinterpret_cast<uint8_t *>(&ushot16x4.z), &ptr[4]);
    copy_data<2u>(reinterpret_cast<uint8_t *>(&ushot16x4.w), &ptr[6]);
    store_as<ushort4>(dest, ushot16x4);
}

template <>
__device__ inline void copy_data<16u>(uint8_t *dest, const uint8_t *ptr) {
    copy_data<8u>(dest + 0, ptr + 0);
    copy_data<8u>(dest + 8, ptr + 8);
}
